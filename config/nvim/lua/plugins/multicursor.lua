return {
  {
    ...
    config = function()
      ...
      set({ "n", "x" }, "<leader><up>", function()
        mc.lineAddCursor(-1)
      end, { desc = "Line add cursor previous" })
      set({ "n", "x" }, "<leader><down>", function()
        mc.lineAddCursor(1)
      end, { desc = "Line add cursor next" })
      set({ "n", "x" }, "<leader><s-up>", function()
        mc.lineSkipCursor(-1)
      end, { desc = "Line skip cursor previous" })
      set({ "n", "x" }, "<leader><s-down>", function()
        mc.lineSkipCursor(1)
      end, { desc = "Line skip cursor next" })

      -- Add or skip adding a new cursor by matching word/selection
      set({ "n", "x" }, "<leader><right>", function()
        mc.matchAddCursor(1)
      end, { desc = "Match add cursor next" })
      set({ "n", "x" }, "<leader><left>", function()
        mc.matchAddCursor(-1)
      end, { desc = "Match add cursor previous" })
      set({ "n", "x" }, "<leader><s-right>", function()
        mc.matchSkipCursor(1)
      end, { desc = "Match skip cursor next" })
      set({ "n", "x" }, "<leader><s-left>", function()
        mc.matchSkipCursor(-1)
